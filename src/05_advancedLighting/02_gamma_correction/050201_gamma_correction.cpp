...
int main(int argc, char **argv)
{
    ...
    // 额外加载一个以sRGB格式加载的地板纹理
    unsigned int linearFloorTexture = loadTexture(
        floorTexturePath.c_str(),
        GL_REPEAT,
        GL_REPEAT,
        ...
        std::nullopt,
        true,
        GL_SRGB // 以sRGB格式加载纹理，实现自动的纹理gamma校正
    );
    
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, floorTexture);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, linearFloorTexture);
    ...
    static unsigned int gamma_correction_mode = 1; // 0: disable, 1: enable opengl built-in sRGB, 2: manual in shader 
    const char* gamma_correction_items[] = { "Disable", "Enable (OpenGL sRGB)", "Enable (Manual in Shader)" };
    bool manualTex2Linear = false; // 是否手动将采样的纹理从sRGB空间转换为线性空间
    if(manualTex2Linear){
        shader.use();
        shader.setBool("manualTex2Linear", true);
    }
    else{
        shader.use();
        shader.setBool("manualTex2Linear", false);
    }
    ...
    float pointLightConstant = 0.0f;
    float pointLightLinear = 0.0f;
    float pointLightQuadratic = 1.f;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // gamma correction即将color值开2.2次方再输出到帧缓冲，因为显示器会自动将sRGB空间的值做2.2次方处理再显示
        if(gamma_correction_mode == 0){
            glDisable(GL_FRAMEBUFFER_SRGB);
            shader.use();
            shader.setBool("gammaCorrection", false);
            // std::cout << "Gamma Correction Disabled" << std::endl;
        }
        else if(gamma_correction_mode == 1){
            glEnable(GL_FRAMEBUFFER_SRGB);
            shader.use();
            shader.setBool("gammaCorrection", false);
            // std::cout << "Gamma Correction Enabled (OpenGL sRGB)" << std::endl;
        }
        else{
            glDisable(GL_FRAMEBUFFER_SRGB);
            shader.use();
            shader.setBool("gammaCorrection", true);
            // std::cout << "Gamma Correction Enabled (Manual in Shader)" << std::endl;
        }
        ...
        if(manualTex2Linear){
            shader.setInt("material.texture_diffuse", 0); // 手动在shader中将采样的纹理从sRGB空间转换为线性空间
        }else{
            shader.setInt("material.texture_diffuse", 1); // 用已经读到的线性纹理
        }
        ...
        glBindVertexArray(0);
        glDisable(GL_FRAMEBUFFER_SRGB); // 渲染其他东西不需要gamma correction，已经在sRGB空间了。 
                                // 实际上地板纹理也是sRGB的，但做光照计算时需要线性空间，所以在frag中采样后开2.2次方转为线性空间再计算光照。
        ...
        ImGui::NewFrame();
        {
            ...
            // Gamma Correction
            ImGui::Separator();
            ImGui::Text("Gamma Correction Mode");
            ImGui::Combo("##gamma_correction_mode", (int *)&gamma_correction_mode, gamma_correction_items, IM_ARRAYSIZE(gamma_correction_items));
            ImGui::Checkbox("手动sRGB2Linear", &manualTex2Linear);
