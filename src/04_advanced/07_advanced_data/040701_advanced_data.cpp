...
#pragma region  //设置数据三种方式
    // 1. 使用 glBufferData 一次性分配和初始化缓冲区数据
    // glBufferData(GL_ARRAY_BUFFER, sizeof(skyboxCubeVertices), &skyboxCubeVertices, GL_STATIC_DRAW);
    
    // // 2. 使用 glBufferData 分配内存，然后使用 glBufferSubData 分段复制数据
    // glBufferData(GL_ARRAY_BUFFER, sizeof(skyboxCubeVertices), nullptr, GL_STATIC_DRAW); // 先分配内存但不复制数据
