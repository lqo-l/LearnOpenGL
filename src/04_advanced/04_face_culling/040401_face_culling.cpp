...
int main(int argc, char **argv)
{
    ...
    static bool faceCulling = true; // 是否开启面剔除
    static bool cull_front = false; // 是否剔除正面
    ...
    if(faceCulling){
        ...
        if(cull_front){
            ...
            glCullFace(GL_BACK); // GL_BACK仅剔除背向面，默认值。此外还有：GL_FRONT,GL_FRONT_AND_BACK
        }
        glFrontFace(GL_CCW); // counter clock-wise，逆时针认为是正面，默认值。GL_CW为顺时针正面。 
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ImGui::NewFrame();
        {
            ...
            // Culling
            ImGui::Separator();
            ImGui::Text("face cull设置");
            if(ImGui::Checkbox("开启face cull", &faceCulling)){
                if(faceCulling){
                    glEnable(GL_CULL_FACE);
                }else{
            ...
            if(ImGui::Checkbox("剔除正面", &cull_front)){
                if(cull_front){
                    glCullFace(GL_FRONT); 
                }else{
                    glCullFace(GL_BACK); 
