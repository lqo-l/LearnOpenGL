...
int main(int argc, char **argv)
{
    ...
    // 着色器
    Shader shader(getAssetAbsPath(argv[0], "model.vert"), getAssetAbsPath(argv[0], "model.frag"));
    Shader skyboxShader(getAssetAbsPath(argv[0], "skybox_optim.vert"), getAssetAbsPath(argv[0], "skybox.frag"));
...
#pragma endregion 
    ...
    // 这里模型用到3类纹理：漫反射(dif)，镜面反射(spec)，切线空间法线贴图(ddn,bump)，所以天空盒放到纹理单元4，不与模型冲突。
    glActiveTexture(GL_TEXTURE4);
    glBindTexture(GL_TEXTURE_CUBE_MAP, skyboxSeaTexture);

    /// --- model ---
    Model nanosuitModel(getAssetAbsPath(argv[0], "assets/04_advanced/models/nanosuit_reflection/nanosuit.obj").c_str());
    nanosuitModel.Debug();
    ...
    static int effect = 1; // 0: 无环境映射， 1：反射， 2：折射
    // 光照参数
    static float Ia = 1.0f; // 环境光强度
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shader.setFloat("Ia", Ia);

        nanosuitModel.Draw(shader);

        // --- render skybox：通过vert中设置skybox NDC深度为1，可以将天空盒靠后渲染，利用early-z技术可以减少frag开销 ---
        glDepthFunc(GL_LEQUAL); // 修改深度测试函数,让天空盒=1也能通过测试
        skyboxShader.use();
        skyboxShader.setMat4("view", glm::mat4(glm::mat3(view))); // 去掉平移部分,让skybox立方体永远在相机位置
        ...
        ImGui::NewFrame();
        {
            ...
            ImGui::Separator();
            ImGui::Text("光照参数");
            ImGui::DragFloat("环境光强度 Ia", &Ia, 0.01f, 0.0f, 1.0f);
            ...
            ImGui::Separator();
            ImGui::Text("环境映射选项");
            ImGui::RadioButton("无环境映射", &effect, 0);
            ImGui::RadioButton("反射", &effect, 1);
            ImGui::RadioButton("折射", &effect, 2);
