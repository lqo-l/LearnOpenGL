...
int main(int argc, char **argv)
{
    ...
    // 着色器
    Shader common_shader(getAssetAbsPath(argv[0], "uniform_instancing.vert"), getAssetAbsPath(argv[0], "common.frag"));
    ...
    int row = 10;  // 10*10个quad
    int col = 10;
    std::vector<glm::vec2> translations(row * col);
    float init = 2.0f / row / 2; // 第一个quad相对于边界的偏移, 0.1
    float interval = 2.f / row; // 间隔
    for(int y = 0; y < row; y++){
        for(int x = 0; x < col; x++){
            translations[y*row + x] = glm::vec2(-1.f + init + x * interval, -1.f + init + y * interval); 
        }
    }

    common_shader.use();
    for(int i = 0; i < translations.size(); i++){
        common_shader.setVec2("offsets[" + std::to_string(i) +"]", translations[i]);
    }
