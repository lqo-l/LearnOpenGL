...
int main(int argc, char **argv)
{
    ...
    // 着色器
    Shader shader(getAssetAbsPath(argv[0], "1.1.depth_testing.vert"), getAssetAbsPath(argv[0], "1.2.unlinear_depth.frag"));
    ...
    shader.use();
    shader.setBool("visualLinearDepth", false); // 初始化
    shader.setBool("visualUnlinearDepth", true );
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ImGui::NewFrame();
        {
            ...
            if (ImGui::RadioButton("可视化线性深度", &depthMode, 0) ||
                ImGui::RadioButton("可视化非线性深度", &depthMode, 1) ||
                ImGui::RadioButton("可视化关闭", &depthMode, 2))
            {
                shader.use();
                shader.setBool("visualLinearDepth", depthMode == 0);
                shader.setBool("visualUnlinearDepth", depthMode == 1);
            }
